function [mre,Fmre] = andmre(x,y,xy,p,varargin)
%andmre Multisensory response enhancement for a bisensory AND task.
%   MRE = ANDMRE(X,Y,XY) returns the multisensory response enhancement
%   (MRE) for a bisensory AND task, quantified by the area between the CDFs
%   of the slower of the unisensory RT distributions X and Y, and the
%   bisensory RT distribution XY, normalized by the former (Crosse et al.,
%   2019). X, Y and XY can have different lengths. This function treats
%   NaNs as missing values, and ignores them.
%
%   [...] = ANDMRE(...,P) uses the probabilities P to compute the CDF
%   quantiles. P is a vector of decimal values between 0 and 1 inclusive
%   (default=0.05:0.1:0.95).
%
%   [...,FMRE] = ANDMRE(...) returns the MRE at every CDF quantile. If the
%   mean or median method is chosen, then FMRE is returned empty.
%
%   [...] = ANDMRE(...,'PARAM1',VAL1,'PARAM2',VAL2,...) specifies
...
%
%   See also ANDMRE3, ANDBENEFIT, ANDGAIN, ANDMODEL, TPERMTEST, EFFECTSIZE.
%
