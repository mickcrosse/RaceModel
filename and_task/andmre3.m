function [mre,Fmre] = andmre3(x,y,z,xyz,p,varargin)
%andmre3 Multisensory response enhancement for a trisensory AND task.
%   MRE = ANDMRE3(X,Y,Z,XYZ) returns the multisensory response enhancement
%   (MRE) for a trisensory AND task, quantified by the area between the
%   CDFs of the slower of the unisensory RT distributions X, Y and Z, and
%   the trisensory RT distribution XYZ, normalized by the former (Crosse et
%   al., 2019). X, Y, Z and XYZ can have different lengths. This function
%   treats NaNs as missing values, and ignores them.
%
%   [...] = ANDMRE3(...,P) uses the probabilities P to compute the CDF
%   quantiles. P is a vector of decimal values between 0 and 1 inclusive
%   (default=0.05:0.1:0.95).
%
%   [...,FMRE] = ANDMRE3(...) returns the MRE at every CDF quantile. If the
%   mean or median method is chosen, then FMRE is returned empty.
%
%   [...] = ANDMRE3(...,'PARAM1',VAL1,'PARAM2',VAL2,...) specifies
...
%
%   See also ANDMRE, ANDBENEFIT3, ANDGAIN3, TPERMTEST, EFFECTSIZE.
%
